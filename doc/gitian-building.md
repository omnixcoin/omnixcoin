...
*Setup instructions for a Gitian build of Omnix Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Omnix
...
- [Building Omnix Core](#building-omnix-core)
...
Clone the git repositories for omnix and Gitian.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/omnixofficial/Omnix-Core --recursive
git clone https://github.com/omnixofficial/gitian.sigs.git
```
...
```bash
sudo cp /home/debian/omnix/contrib/gitian-descriptors/xenial /usr/share/debootstrap/scripts
```
...
in the omnix repository under 'Fetch and create inputs' to install sources which require
...
To build Omnix Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the omnix repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/omnix/.git/
...
For example:
```bash
URL=https://github.com/laanwj/omnix.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit omnix=${COMMIT} --url omnix=${URL} ../omnix/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit omnix=${COMMIT} --url omnix=${URL} ../omnix/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit omnix=${COMMIT} --url omnix=${URL} ../omnix/contrib/gitian-descriptors/gitian-osx.yml
# if wanting to use a different version of eth-cpp-omnix:
./bin/gbuild --commit omnix=${COMMIT},cpp-eth-omnix=${ETHCOMMIT} --url omnix=${URL},cpp-eth-omnix=${ETHURL} ../omnix/contrib/gitian-descriptors/gitian-linux.yml
```
...
and the omnix git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../omnix/contrib/gitian-descriptors/*|sort|uniq )
...
cd /some/root/path/
git clone https://github.com/omnixofficial/Omnix-Core-detached-sigs.git

BTCPATH=/some/root/path/omnix
SIGPATH=/some/root/path/omnix-detached-sigs

./bin/gbuild --url omnix=${BTCPATH},signature=${SIGPATH} ../omnix/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/omnix-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/omnix-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/omnix-osx-build.assert
```
...
[omnixproject/gitian.sigs](https://github.com/omnixofficial/gitian.sigs/) repository, or if that's not possible create a pull
request. You can also mail the files to Jordan Earls (earlz@omnix.org) and he will commit them.