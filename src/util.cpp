...
const char * const BITCOIN_CONF_FILENAME = "omnix.conf";
const char * const BITCOIN_PID_FILENAME = "omnixd.pid";
...
{
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\Omnix
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\Omnix
    // Mac: ~/Library/Application Support/Omnix
    // Unix: ~/.omnix
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Omnix";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Omnix";
#else
    // Unix
    return pathRet / ".omnix";
#endif
#endif
}
...
std::string CopyrightHolders(const std::string& strPrefix)
{
    ...
    if (strprintf(COPYRIGHT_HOLDERS, COPYRIGHT_HOLDERS_SUBSTITUTION).find("Omnix Core") == std::string::npos) {
        strCopyrightHolders += "\n" + strPrefix + "The Omnix Core developers";
    }
