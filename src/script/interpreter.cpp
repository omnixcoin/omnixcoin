...
#define omnixtacktop(i)  (omnixtack.at(omnixtack.size()+(i)))
        ...
        while (pc < pend)
        {
            ...
            switch (opcode)
            {
                ...
                case OP_TOOMNIXTACK:
                {
                    if (stack.size() < 1)
                        return set_error(serror, SCRIPT_ERR_INVALID_STACK_OPERATION);
                    omnixtack.push_back(stacktop(-1));
                    popstack(stack);
                }
                break;

                case OP_FROMOMNIXTACK:
                {
                    if (omnixtack.size() < 1)
                        return set_error(serror, SCRIPT_ERR_INVALID_OMNIXTACK_OPERATION);
                    stack.push_back(omnixtacktop(-1));
                    popstack(omnixtack);
                }
                break;
            ...
            // Size limits
            if (stack.size() + omnixtack.size() > MAX_STACK_SIZE)
