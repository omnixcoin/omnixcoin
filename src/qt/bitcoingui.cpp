...
#include <qt/omnixversionchecker.h>
    ...
    platformStyle(_platformStyle)
{
    ...
    omnixVersionChecker = new OmnixVersionChecker(this);
...
void BitcoinGUI::createActions()
{
    ...
    sendCoinsAction->setStatusTip(tr("Send coins to a Omnix address"));
    ...
    receiveCoinsAction->setStatusTip(tr("Request payments (generates QR codes and omnix: URIs)"));
    ...
    OMNIXTokenAction = new QAction(platformStyle->MultiStatesIcon(":/icons/omnixtoken"), tr("&OMNIX Tokens"), this);
    OMNIXTokenAction->setStatusTip(tr("OMNIX Tokens (send, receive or add Tokens in list)"));
    OMNIXTokenAction->setToolTip(OMNIXTokenAction->statusTip());
    OMNIXTokenAction->setCheckable(true);
    OMNIXTokenAction->setShortcut(QKeySequence(Qt::ALT + Qt::Key_6));
    tabGroup->addAction(OMNIXTokenAction);
    ...
    signMessageAction->setStatusTip(tr("Sign messages with your Omnix addresses to prove you own them"));
    verifyMessageAction = new QAction(platformStyle->MenuColorIcon(":/icons/verify"), tr("&Verify message..."), this);
    verifyMessageAction->setStatusTip(tr("Verify messages to ensure they were signed with specified Omnix addresses"));
    ...
    openAction->setStatusTip(tr("Open a omnix: URI or payment request"));//OMNIX_LINE

    showHelpMessageAction = new QAction(platformStyle->MenuColorIcon(":/icons/info"), tr("&Command-line options"), this);
    showHelpMessageAction->setMenuRole(QAction::NoRole);
    showHelpMessageAction->setStatusTip(tr("Show the %1 help message to get a list with possible Omnix command-line options").arg(tr(PACKAGE_NAME)));
...
void BitcoinGUI::createToolBars()
{
    if(walletFrame)
    {
        ...
        appNavigationBar->mapGroup(OMNIXTokenAction, tokenActions);
...
void BitcoinGUI::setClientModel(ClientModel *_clientModel)
{
    ...
    {
        // Check for updates
        if(_clientModel->getOptionsModel()->getCheckForUpdates() && omnixVersionChecker->newVersionAvailable())
        {
            QString link = QString("<a href=%1>%2</a>").arg(OMNIX_RELEASES, OMNIX_RELEASES);
            QString message(tr("New version of Omnix wallet is available on the Omnix source code repository: <br /> %1. <br />It is recommended to download it and update this application").arg(link));
...
void BitcoinGUI::gotoSendTokenPage()
{
    ...
    OMNIXTokenAction->setChecked(true);
...
void BitcoinGUI::updateNetworkState()
{
    ...
    if (m_node.getNetworkActive()) {
        tooltip = tr("%n active connection(s) to Omnix network", "", count) + QString(".<br>") + tr("Click to disable network activity.");
    } else {
...
{
    QString strTitle = tr("Omnix"); // default title
