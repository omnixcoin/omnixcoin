...
bool ToHash160(const std::string& strOmnixAddress, std::string& strHash160)
{
    CTxDestination omnixAddress = DecodeDestination(strOmnixAddress);
    if(!IsValidDestination(omnixAddress))
        return false;
    const CKeyID * keyid = boost::get<CKeyID>(&omnixAddress);
    if(keyid){
...
bool ToOmnixAddress(const std::string& strHash160, std::string& strOmnixAddress)
{
    uint160 key(ParseHex(strHash160.c_str()));
    CKeyID keyid(key);
    CTxDestination omnixAddress = keyid;
    if(IsValidDestination(omnixAddress)){
        strOmnixAddress = EncodeDestination(omnixAddress);
        return true;
    }
    return false;
}
...
bool Token::execEvents(int64_t fromBlock, int64_t toBlock, int func, std::vector<TokenEvent> &tokenEvents)
{
    ...
    for(int i = 0; i < list.size(); i++)
    {
        ...
        for(int i = 0; i < listLog.size(); i++)
        {
            ...
            ToOmnixAddress(tokenEvent.sender, tokenEvent.sender);
            tokenEvent.receiver = topicsList[2].toString().toStdString().substr(24);
            ToOmnixAddress(tokenEvent.receiver, tokenEvent.receiver);
