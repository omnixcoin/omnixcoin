...
#define paternVersion "omnix-([0-9]+\\.)?([0-9]+\\.)?([0-9]+)-"

OmnixVersionChecker::OmnixVersionChecker(QObject *parent) : QObject(parent)
{
    currentVersion = Version(CLIENT_VERSION_MAJOR, CLIENT_VERSION_MINOR, CLIENT_VERSION_REVISION);
}

OmnixVersionChecker::~OmnixVersionChecker()
{

}

bool OmnixVersionChecker::newVersionAvailable()
{
    Version maxReleaseVersion = getMaxReleaseVersion();
    return maxReleaseVersion > currentVersion;
}

QList<Version> OmnixVersionChecker::getVersions()
{
    QNetworkAccessManager manager;
    QNetworkReply *response = manager.get(QNetworkRequest(QUrl(OMNIX_RELEASES)));
    QEventLoop event;
...
Version OmnixVersionChecker::getMaxReleaseVersion()
{
