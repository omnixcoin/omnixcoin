...
inline void initState(){
    ...
    const std::string dirOmnix = pathTemp.string();
    const dev::h256 hashDB(dev::sha3(dev::rlp("")));
    globalState = std::unique_ptr<OmnixState>(new OmnixState(dev::u256(0), OmnixState::openDB(dirOmnix, hashDB, dev::WithExisting::Trust), dirOmnix + "/omnixDB", dev::eth::BaseState::Empty));
...
inline dev::Address createOmnixAddress(dev::h256 hashTx, uint32_t voutNumber){
...
inline OmnixTransaction createOmnixTransaction(valtype data, dev::u256 value, dev::u256 gasLimit, dev::u256 gasPrice, dev::h256 hashTransaction, dev::Address recipient, int32_t nvout = 0){
    OmnixTransaction txEth;
    if(recipient == dev::Address()){
        txEth = OmnixTransaction(value, gasPrice, gasLimit, data, dev::u256(0));
    } else {
        txEth = OmnixTransaction(value, gasPrice, gasLimit, recipient, data, dev::u256(0));
    }
...
inline std::pair<std::vector<ResultExecute>, ByteCodeExecResult> executeBC(std::vector<OmnixTransaction> txs){
