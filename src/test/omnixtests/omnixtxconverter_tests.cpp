...
void checkResult(bool isCreation, std::vector<OmnixTransaction> results, uint256 hash){
...
void runTest(bool isCreation, size_t n, CScript& script1, CScript script2 = CScript()){
    ...
    OmnixTxConverter converter(transaction, NULL);
    ExtractOmnixTX omnixTx;
    BOOST_CHECK(converter.extractionOmnixTransactions(omnixTx));
    std::vector<OmnixTransaction> result = omnixTx.first;
    if(script2 == CScript()){
...
BOOST_FIXTURE_TEST_SUITE(omnixtxconverter_tests, TestingSetup)
