...
#include <omnix/storageresults.h>


extern std::unique_ptr<OmnixState> globalState;
...
using ExtractOmnixTX = std::pair<std::vector<OmnixTransaction>, std::vector<EthTransactionParams>>;
///////////////////////////////////////////
...
static const size_t MAX_CONTRACT_VOUTS = 1000; // omnix
...
void EnforceContractVoutLimit(ByteCodeExecResult& bcer, ByteCodeExecResult& bcerOut, const dev::h256& oldHashOmnixRoot,
    const dev::h256& oldHashStateRoot, const std::vector<OmnixTransaction>& transactions);
...
class OmnixTxConverter{

public:

    OmnixTxConverter(CTransaction tx, CCoinsViewCache* v = NULL, const std::vector<CTransactionRef>* blockTxs = NULL) : txBit(tx), view(v), blockTransactions(blockTxs){}

    bool extractionOmnixTransactions(ExtractOmnixTX& omnixTx);
...
private:
    ...
    OmnixTransaction createEthTX(const EthTransactionParams& etp, const uint32_t nOut);
...
public:
    ...
    ByteCodeExec(const CBlock& _block, std::vector<OmnixTransaction> _txs, const uint64_t _blockGasLimit, CBlockIndex* _pindex) : txs(_txs), block(_block), blockGasLimit(_blockGasLimit), pindex(_pindex) {}
