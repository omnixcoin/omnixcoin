...
{
    const std::string URL_SOURCE_CODE = "<https://github.com/omnixofficial/Omnix-Core>";
    const std::string URL_WEBSITE = "<https://altiucoin.com>";
    ...
    while (!fLoaded && !ShutdownRequested()) {
        ...
        do {
            ...
            try {
                ...
                dev::eth::Ethash::init();
                fs::path omnixStateDir = GetDataDir() / "stateOmnix";
                bool fStatus = fs::exists(omnixStateDir);
                const std::string dirOmnix(omnixStateDir.string());
                const dev::h256 hashDB(dev::sha3(dev::rlp("")));
                dev::eth::BaseState existsOmnixstate = fStatus ? dev::eth::BaseState::PreExisting : dev::eth::BaseState::Empty;
                globalState = std::unique_ptr<OmnixState>(new OmnixState(dev::u256(0), OmnixState::openDB(dirOmnix, hashDB, dev::WithExisting::Trust), dirOmnix, existsOmnixstate));
                dev::eth::ChainParams cp((dev::eth::genesisInfo(dev::eth::Network::omnixMainNetwork)));
                globalSealEngine = std::unique_ptr<dev::eth::SealEngineFace>(cp.createSealEngine());

                pstorageresult.reset(new StorageResults(omnixStateDir.string()));
                if (fReset) {
