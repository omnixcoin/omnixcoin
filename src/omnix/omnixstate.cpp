...
#include <omnix/omnixstate.h>

using namespace std;
using namespace dev;
using namespace dev::eth;

OmnixState::OmnixState(u256 const& _accountStartNonce, OverlayDB const& _db, const string& _path, BaseState _bs) :
        State(_accountStartNonce, _db, _bs) {
            dbUTXO = OmnixState::openDB(_path + "/omnixDB", sha3(rlp("")), WithExisting::Trust);
	        stateUTXO = SecureTrieDB<Address, OverlayDB>(&dbUTXO);
}

OmnixState::OmnixState() : dev::eth::State(dev::Invalid256, dev::OverlayDB(), dev::eth::BaseState::PreExisting) {
    dbUTXO = OverlayDB();
    stateUTXO = SecureTrieDB<Address, OverlayDB>(&dbUTXO);
}

ResultExecute OmnixState::execute(EnvInfo const& _envInfo, SealEngineFace const& _sealEngine, OmnixTransaction const& _t, Permanence _p, OnOpFunc const& _onOp){

    assert(_t.getVersion().toRaw() == VersionVM::GetEVMDefault().toRaw());

    addBalance(_t.sender(), _t.value() + (_t.gas() * _t.gasPrice()));
    newAddress = _t.isCreation() ? createOmnixAddress(_t.getHashWith(), _t.getNVout()) : dev::Address();
    ...
    try{
        ...
        if (_p == Permanence::Reverted){
            ...
            omnix::commit(cacheUTXO, stateUTXO, m_cache);
            cacheUTXO.clear();
...
std::unordered_map<dev::Address, Vin> OmnixState::vins() const // temp
{
...
void OmnixState::transferBalance(dev::Address const& _from, dev::Address const& _to, dev::u256 const& _value) {
...
Vin const* OmnixState::vin(dev::Address const& _a) const
{
    return const_cast<OmnixState*>(this)->vin(_a);
}

Vin* OmnixState::vin(dev::Address const& _addr)
{
...
// void OmnixState::commit(CommitBehaviour _commitBehaviour)
// {
...
void OmnixState::kill(dev::Address _addr)
{
...
void OmnixState::addBalance(dev::Address const& _id, dev::u256 const& _amount)
{
...
void OmnixState::deleteAccounts(std::set<dev::Address>& addrs){
...
void OmnixState::updateUTXO(const std::unordered_map<dev::Address, Vin>& vins){
    for(auto& v : vins){
...
void OmnixState::printfErrorLog(const dev::eth::TransactionException er){
    std::stringstream ss;
    ss << er;
