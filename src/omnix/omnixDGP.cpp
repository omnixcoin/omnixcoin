#include <omnix/omnixDGP.h>

void OmnixDGP::initDataEIP158(){
...
bool OmnixDGP::checkLimitSchedule(const std::vector<uint32_t>& defaultData, const std::vector<uint32_t>& checkData){
...
dev::eth::EVMSchedule OmnixDGP::getGasSchedule(unsigned int blockHeight){
    clear();
...
uint64_t OmnixDGP::getUint64FromDGP(unsigned int blockHeight, const dev::Address& contract, std::vector<unsigned char> data){
    uint64_t value = 0;
...
uint32_t OmnixDGP::getBlockSize(unsigned int blockHeight){
    clear();
...
uint64_t OmnixDGP::getMinGasPrice(unsigned int blockHeight){
    clear();
...
uint64_t OmnixDGP::getBlockGasLimit(unsigned int blockHeight){
    clear();
...
bool OmnixDGP::initStorages(const dev::Address& addr, unsigned int blockHeight, std::vector<unsigned char> data){
    initStorageDGP(addr);
...
void OmnixDGP::initStorageDGP(const dev::Address& addr){
    storageDGP = state->storage(addr);
}

void OmnixDGP::initStorageTemplate(const dev::Address& addr){
    storageTemplate = state->storage(addr);
}

void OmnixDGP::initDataTemplate(const dev::Address& addr, std::vector<unsigned char>& data){
    dataTemplate = CallContract(addr, data)[0].execRes.output;
}

void OmnixDGP::createParamsInstance(){
...
dev::Address OmnixDGP::getAddressForBlock(unsigned int blockHeight){
...
void OmnixDGP::parseStorageScheduleContract(std::vector<uint32_t>& uint32Values){
...
void OmnixDGP::parseDataScheduleContract(std::vector<uint32_t>& uint32Values){
...
void OmnixDGP::parseStorageOneUint64(uint64_t& value){
...
void OmnixDGP::parseDataOneUint64(uint64_t& value){
    if(dataTemplate.size() == 32){
        value = dev::toUint64(dev::u256(dev::h256(dataTemplate)));
    }
}

dev::eth::EVMSchedule OmnixDGP::createEVMSchedule(){
    dev::eth::EVMSchedule schedule = dev::eth::EIP158Schedule;
