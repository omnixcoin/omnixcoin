...
std::unique_ptr<CBlockTemplate> BlockAssembler::CreateNewBlock(const CScript& scriptPubKeyIn, bool fMineWitnessTx, bool fProofOfStake, int64_t* pTotalFees, int32_t txProofTime, int32_t nTimeLimit)
{
    ...
    globalSealEngine->setOmnixSchedule(omnixDGP.getGasSchedule(nHeight));
    uint32_t blockSizeDGP = omnixDGP.getBlockSize(nHeight);
    minGasPrice = omnixDGP.getMinGasPrice(nHeight);
    ...
    hardBlockGasLimit = omnixDGP.getBlockGasLimit(nHeight);
...
bool BlockAssembler::AttemptToAddContractToBlock(CTxMemPool::txiter iter, uint64_t minGasPrice) {
    ...
    OmnixTxConverter convert(iter->GetTx(), NULL, &pblock->vtx);

    ExtractOmnixTX resultConverter;
    if(!convert.extractionOmnixTransactions(resultConverter)){
    ...
    for(OmnixTransaction omnixTransaction : omnixTransactions){
        txGas += omnixTransaction.gas();
        if(txGas > txGasLimit) {
            // Limit the tx gas limit by the soft limit if such a limit has been specified.
            return false;
        }

        if(bceResult.usedGas + omnixTransaction.gas() > softBlockGasLimit){
            //if this transaction's gasLimit could cause block gas limit to be exceeded, then don't add it
            return false;
        }
        if(omnixTransaction.gasPrice() < minGasPrice){
    ...
    ByteCodeExec exec(*pblock, omnixTransactions, hardBlockGasLimit, chainActive.Tip());
...
void ThreadStakeMiner(CWallet *pwallet, CConnman* connman)
{
    ...
    std::string threadName = "omnixstake";
...
void StakeOmnixs(bool fStake, CWallet *pwallet, CConnman* connman, boost::thread_group*& stakeThread)
{
