...
#if defined(NDEBUG)
# error "Omnix cannot be compiled without assertions."
#endif
...
std::unique_ptr<OmnixState> globalState;
...
bool fAddressIndex = false; // omnix
#endif
...
const std::string strMessageMagic = "Omnix Signed Message:\n";
                              ...
                              bool bypass_limits, const CAmount& nAbsurdFee, std::vector<COutPoint>& coins_to_uncache, bool test_accept, bool rawTx)
{
...

    {
        ...
        if(tx.HasCreateOrCall()){
            ...
            uint64_t minGasPrice = omnixDGP.getMinGasPrice(chainActive.Tip()->nHeight + 1);
            uint64_t blockGasLimit = omnixDGP.getBlockGasLimit(chainActive.Tip()->nHeight + 1);
            size_t count = 0;
            for(const CTxOut& o : tx.vout)
                count += o.scriptPubKey.HasOpCreate() || o.scriptPubKey.HasOpCall() ? 1 : 0;
            OmnixTxConverter converter(tx, NULL);
            ExtractOmnixTX resultConverter;
            if(!converter.extractionOmnixTransactions(resultConverter)){
            ...
            std::vector<OmnixTransaction> omnixTransactions = resultConverter.first;
            std::vector<EthTransactionParams> omnixETP = resultConverter.second;

            dev::u256 sumGas = dev::u256(0);
            dev::u256 gasAllTxs = dev::u256(0);
            for(OmnixTransaction omnixTransaction : omnixTransactions){
                sumGas += omnixTransaction.gas() * omnixTransaction.gasPrice();
                ...
                if(txMinGasPrice != 0) {
                    txMinGasPrice = std::min(txMinGasPrice, omnixTransaction.gasPrice());
                } else {
                    txMinGasPrice = omnixTransaction.gasPrice();
                }
                VersionVM v = omnixTransaction.getVersion();
                if(v.format!=0)
                ...
                if(omnixTransaction.gas() < gArgs.GetArg("-minmempoolgaslimit", MEMPOOL_MIN_GAS_LIMIT))
                ...
                if(omnixTransaction.gas() < MINIMUM_GAS_LIMIT && v.rootVM != 0)
                ...
                if(omnixTransaction.gas() > UINT32_MAX)
                ...
                gasAllTxs += omnixTransaction.gas();
                ...
                if(v.rootVM!=0 && (uint64_t)omnixTransaction.gasPrice() < minGasPrice)
            ...
            if(!CheckMinGasPrice(omnixETP, minGasPrice))
                return state.DoS(100, false, REJECT_INVALID, "bad-txns-small-gasprice");

            if(count > omnixTransactions.size())
        ...
        for (CTxMemPool::txiter it : allConflicting)
        {
            LogPrint(BCLog::MEMPOOL, "replacing tx %s with %s for %s OMNIX additional fees, %d delta bytes\n",
...
DisconnectResult CChainState::DisconnectBlock(const CBlock& block, const CBlockIndex* pindex, CCoinsViewCache& view, bool* pfClean)
{
    ...
    globalState->setRoot(uintToh256(pindex->pprev->hashStateRoot)); // omnix
    globalState->setRootUTXO(uintToh256(pindex->pprev->hashUTXORoot)); // omnix
...
std::vector<ResultExecute> CallContract(const dev::Address& addrContract, std::vector<unsigned char> opcode, const dev::Address& sender, uint64_t gasLimit){
    ...
    OmnixTransaction callTransaction(0, 1, dev::u256(gasLimit), addrContract, opcode, dev::u256(0));
    callTransaction.forceSender(senderAddress);
    callTransaction.setVersion(VersionVM::GetEVMDefault());


    ByteCodeExec exec(block, std::vector<OmnixTransaction>(1, callTransaction), blockGasLimit, pblockindex);
...
void writeVMlog(const std::vector<ResultExecute>& res, const CTransaction& tx, const CBlock& block){
    boost::filesystem::path omnixDir = GetDataDir() / "vmExecLogs.json";
    std::stringstream ss;
    if(fIsVMlogFile){
        ss << ",";
    } else {
        std::ofstream file(omnixDir.string(), std::ios::out | std::ios::app);
        file << "{\"logs\":[]}";
        file.close();
    }
    ...
    std::ofstream file(omnixDir.string(), std::ios::in | std::ios::out);
...
bool OmnixTxConverter::extractionOmnixTransactions(ExtractOmnixTX& omnixtx){
    std::vector<OmnixTransaction> resultTX;
    ...
    omnixtx = std::make_pair(resultTX, resultETP);
    return true;
}

bool OmnixTxConverter::receiveStack(const CScript& scriptPubKey){
...
bool OmnixTxConverter::parseEthTXParams(EthTransactionParams& params){
    try{
...
OmnixTransaction OmnixTxConverter::createEthTX(const EthTransactionParams& etp, uint32_t nOut){
    OmnixTransaction txEth;
    if (etp.receiveAddress == dev::Address() && opcode != OP_CALL){
        txEth = OmnixTransaction(txBit.vout[nOut].nValue, etp.gasPrice, etp.gasLimit, etp.code, dev::u256(0));
    }
    else{
        txEth = OmnixTransaction(txBit.vout[nOut].nValue, etp.gasPrice, etp.gasLimit, etp.receiveAddress, etp.code, dev::u256(0));
    }
                  ...
                  CCoinsViewCache& view, const CChainParams& chainparams, bool fJustCheck)
{
    ...
    ///////////////////////////////////////////////// // omnix
    OmnixDGP omnixDGP(globalState.get(), fGettingValuesDGP);
    globalSealEngine->setOmnixSchedule(omnixDGP.getGasSchedule(pindex->nHeight + 1));
    uint32_t sizeBlockDGP = omnixDGP.getBlockSize(pindex->nHeight + 1);
    uint64_t minGasPrice = omnixDGP.getMinGasPrice(pindex->nHeight + 1);
    uint64_t blockGasLimit = omnixDGP.getBlockGasLimit(pindex->nHeight + 1);
    ...
    if (block.vtx.empty() || block.vtx.size() > dgpMaxBlockSize || ::GetSerializeSize(block, SER_NETWORK, PROTOCOL_VERSION | SERIALIZE_TRANSACTION_NO_WITNESS) > dgpMaxBlockSize) // omnix
...
///////////////////////////////////////////////////////////////////////////////////////// omnix
        ...
        if(tx.HasCreateOrCall() && !hasOpSpend){
            ...
            OmnixTxConverter convert(tx, &view, &block.vtx);

            ExtractOmnixTX resultConvertOmnixTX;
            if(!convert.extractionOmnixTransactions(resultConvertOmnixTX)){
            ...
            if(!CheckMinGasPrice(resultConvertOmnixTX.second, minGasPrice))
            ...
            dev::u256 gasAllTxs = dev::u256(0);
            ByteCodeExec exec(block, resultConvertOmnixTX.first, blockGasLimit, pindex->pprev);
            ...
            for(OmnixTransaction& qtx : resultConvertOmnixTX.first){
            ...
            {
                for(size_t k = 0; k < resultConvertOmnixTX.first.size(); k ++){
                    ...
                    tri.push_back(TransactionReceiptInfo{block.GetHash(), uint32_t(pindex->nHeight), tx.GetHash(), uint32_t(i), resultConvertOmnixTX.first[k].from(), resultConvertOmnixTX.first[k].to(),
...
bool CChainState::ConnectTip(CValidationState& state, const CChainParams& chainparams, CBlockIndex* pindexNew, const std::shared_ptr<const CBlock>& pblock, ConnectTrace& connectTrace, DisconnectedBlockTransactions &disconnectpool)
{
    ...
    LogPrint(BCLog::BENCH, "  - Load block from disk: %.2fms [%.2fs]\n", (nTime2 - nTime1) * MILLI, nTimeReadFromDisk * MICRO);
    {
        ...
        dev::h256 oldHashStateRoot(globalState->rootHash()); // omnix
        dev::h256 oldHashUTXORoot(globalState->rootHashUTXO()); // omnix
        ...
        if (!rv) {
            ...
            globalState->setRoot(oldHashStateRoot); // omnix
            globalState->setRootUTXO(oldHashUTXORoot); // omnix
...
//////////////////////////////////////////////////////////////////////////
    ...
    for (pindex = chainActive.Tip(); pindex && pindex->pprev; pindex = pindex->pprev) {
        ...
        ///////////////////////////////////////////////////////////////////// // omnix
        uint32_t sizeBlockDGP = omnixDGP.getBlockSize(pindex->nHeight);
        dgpMaxBlockSize = sizeBlockDGP ? sizeBlockDGP : dgpMaxBlockSize;
        updateBlockSizeParams(dgpMaxBlockSize);
        /////////////////////////////////////////////////////////////////////
    ...
    try {
        ...
        while (!blkdat.eof()) {
            ...
            try {
                ...
                // But for Omnix PoS we need to sync this after every block to ensure txdb is populated for
